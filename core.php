 ...
 */
function xapp_ini($key = null, $value = '__NIL__')
{
    if($key !== null)
    {
        if($value !== '__NIL__')
        {
            return ((ini_get($key) !== $value) ? @ini_set($key, $value) : $value);
        }else{
            return ini_get($key);
        }
    }else{
...
if(!function_exists('xapp_config'))
{
     ...
     * function and if not will return/throw default value. the last argument if set to boolean true will return the config
     * object as raw value and not to type object converted
     *
     ...
     * @param bool $raw expects optional boolean value to return config raw
     * @return null|mixed
     * @throws Exception
     */
    function xapp_config($ns, $key = null, $default = null, $raw = false)
    {
        if(xapped('Xapp_Config') && Xapp_Config::created())
        {
            return Xapp_Config::retrieve($ns, $key, $default, $raw);
        }else{
